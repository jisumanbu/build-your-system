...
这是 `build-your-system/assistant` 的 Codex 适配版。
...
- 插件副本
  - `<repo>/assistant-codex`
- 本地插件入口
  - 可选：`~/plugins/build-your-system-assistant`
...
- Vault 工作区提示
  - 可选：在当前 Vault 的 `AGENTS.md` / `AGENTS.override.md` 中声明优先使用本插件
...
- `~/.codex/config.toml` 中开启 `features.plugins = true`
...
python3 /tmp/openai-codex/codex-rs/skills/src/assets/samples/skill-creator/scripts/quick_validate.py \
  <repo>/assistant-codex/skills/assistant-router
```
...
### 5. 在 Vault 里为什么还要有 `AGENTS.md` / `AGENTS.override.md`

这是为了让当前工作区更稳定地优先命中 Codex 版 assistant 工作流。

它不替代主工作区规则，只是补一层本地接入提示。
