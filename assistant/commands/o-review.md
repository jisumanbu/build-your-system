...
- 如果 tag-mapping.md 不存在，使用默认领域标签
...
**【必须执行】** 使用 Skill 工具调用 `/cc-activity {日期}` 获取当日对话活动数据。

⚠️ 这一步不可跳过。cc-activity 分析的是 Claude Code 对话记录，能获取：
- 活动时间线（用户何时在做什么）
- 时间投入统计（每个目标花了多少时间）
- 领域分布

这些数据**无法**从 active.md 获取，必须通过 cc-activity 命令。

如果 cc-activity 执行失败或无数据，再回退到手动询问用户完成情况。
